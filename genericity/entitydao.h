        ...
        : public ICategorizable
{
    int getNewId()
    {
        ...
        if(cnt<3){
            for(int i(0);i<cnt;i++){
                if(i<_objects.at(i)->_id)return i;
            }
            return cnt;
        }
    ...
    void sortById()
    {
        qSort(_objects.begin(),_objects.end(),sortEntities_ById);
    }
...
public:
    EntityDAO(QString cn, QString ext ,QString path )
        :ICategorizable(path,cn),_className(cn),_extension(ext)
    {
        try {
            if(_catsName.count()==0){
                addCategory("default");
            }
            setCategory(0);
        } catch (...) {
